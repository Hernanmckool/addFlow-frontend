...
import { fetchQuotation, updateQuotation } from '@/lib/quotations'
...
export function QuotationEditPage() {
  const { quotationId } = useParams({ from: '/cotizaciones/$quotationId/edit' })
  ...
  const mutation = useMutation({
    mutationFn: (payload: QuotationFormData) =>
    ...
    onError: (error) => {
      if (error instanceof AxiosError && error.response) {
        ...
        setServerError(String(data.message ?? 'Error al actualizar la cotización.'))
      } else {
        setServerError('Error al actualizar la cotización.')
      }
    },
  })
    ...
    values:
      quotation && quotation.status === 'draft'
        ? {
            ...
            valid_until: toDateInput(quotation.valid_until),
  ...
  if (loadingQuotation || loadingClients || loadingAssets) return <LoadingState />
  ...
  // Only draft quotations can be edited (mirrors the backend rule).
  if (quotation.status !== 'draft') {
    return (
      <div>
        <PageHeader
          title={`Cotización ${quotation.number}`}
          description="Esta cotización ya no se puede editar."
          actions={
            <AppButton
              type="button"
              variant="secondary"
              onClick={() => navigate({ to: '/cotizaciones/$quotationId', params: { quotationId } })}
            >
              Ver detalle
            </AppButton>
          }
        />
        <EmptyState
          title="Solo se pueden editar borradores"
          description="Una cotización solo es editable mientras está en estado borrador. Esta ya fue enviada o procesada."
  ...
  // Include assets already in the quotation even if they are no longer "active",
  // so existing selections are preserved.
  const assetOptionMap = new Map<string, string>()
  for (const a of activeAssets) {
    assetOptionMap.set(a.id, a.code ? `[${a.code}] ${a.name}` : a.name)
  }
  for (const item of quotation.items ?? []) {
    if (!assetOptionMap.has(item.asset_id)) {
      assetOptionMap.set(item.asset_id, item.asset.code ? `[${item.asset.code}] ${item.asset.name}` : item.asset.name)
    }
  }
  const assetOptions = Array.from(assetOptionMap, ([value, label]) => ({ value, label }))
  ...
  const status = { label: 'Borrador', badge: 'neutral' as const }

  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <PageHeader
        title="Editar cotización"
        description="Actualiza la propuesta comercial antes de enviarla."
        actions={
          <>
            <AppButton
              type="button"
                ...
                {fields.map((field, index) => (
                  <AppCard key={field.id} variant="default" className="p-4">
                    <div className="mb-3 flex items-center justify-between">
                      <span className="text-[12px] font-semibold text-[#64748B]">Activo {index + 1}</span>
                      {fields.length > 1 && (
                        <button
                          type="button"
          ...
          <div className="flex flex-col gap-6 lg:sticky lg:top-6">
            ...
            <SectionCard title="Resumen">
              <div>
                ...
                <SummaryRow label="Subtotal">
                  <span className="text-[#94A3B8]">Se recalcula al guardar</span>
                </SummaryRow>
                ...
                <SummaryRow label="Moneda">{quotation.currency || CURRENCY}</SummaryRow>
              </div>
            </SectionCard>
            ...
            <SectionCard title="Vista previa">
              <AppCard variant="default" className="p-5">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="font-mono text-[12px] text-[#94A3B8]">{quotation.number}</p>
                ...
                <div className="mt-4 flex items-end justify-between gap-2 border-t border-[#F3F4F6] pt-4">
                  <div>
                    <p className="text-[18px] font-bold leading-none text-[#0F172A]">
                      {quotation.currency || CURRENCY} <span className="text-[#94A3B8]">—</span>
                    </p>
              ...
              <p className="mt-3 text-[12px] text-[#94A3B8]">
                Los montos finales se recalculan al guardar la cotización.
              </p>
            </SectionCard>
          </div>
        </div>
